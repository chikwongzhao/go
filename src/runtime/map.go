...
const (
	...
	// 表示为loadFactorNum/loadFactorDen以允许整数运算。
	loadFactorNum = 13
	loadFactorDen = 2
	...
	// 快速版本不能处理大值 - 文件cmd/compile/internal/gc/walk.go中的快速版本的截止大小必须最大是该元素。
	maxKeySize  = 128
	maxElemSize = 128
	...
	// 数据偏移量应为bmap结构的大小，但需要正确对齐。
	// 对于amd64p32，即使指针是32位，这也意味着64位对齐。
	...
	// 可能会销毁值。我们为特殊标记保留了一些可能性。每个桶(包括其溢出桶，如果有的话)都将拥有全部或不处于evacuated*状态的条目(除了处于evacuate()方法期间，这仅发生在map写入期间，因此此期间任何地方都将访问不到此map)。
